...
# $targetWindowsDir = "Z:"                            # This will be set dynamically to UNC path
...
if ($bios) {   
    ...
    # Setup Azure Files authentication
    ...
    if ($connectTestResult.TcpTestSucceeded) {
        ...
        # Use UNC path directly instead of mapped drive
        $targetWindowsDir = "\\$destSAfile.file.core.windows.net\hash"
        
        # Save credentials for SMB access
        cmd.exe /C "cmdkey /add:`"$destSAfile.file.core.windows.net`" /user:`"Azure\$destSAfile`" /pass:`"$destSAKey`""
        
        Write-Output "Using UNC path: $targetWindowsDir"
    } else {
    ...
    Write-Output "Cleanup Temp credentials"
